...
%% Get ESS population densities for initial conditions. Can either run ESS Analysis or uncomment the hard coded results (Faster and the same).
 ...
 matrixCoefficients(1,:)=[0,0,0,.5,0,2];      %Figure 8-1
 %matrixCoefficients(1,:)=[0,0,0,1/.6,0,1/.4]; %Figure 8-2
...
y0=[0,102,350,452]; %figure 8-1.
%y0=[0,102,10,112]; %figure 8-2.
%y0=[0,102,1,103]; 
...
maxPSAPercent = 0.9; % - This is the lethal tumor burden contour!!!.
%%%PSA_zenith = ESS(matrixIndex,4) * maxPSAPercent;
PSA_zenith = 10000 * maxPSAPercent;
PSA_nadir = PSA_zenith/1.1;
%Advise from improves!!.
...
for time = 2:maxSimulationTime
    ...
    if (AbiOnOffFlag(time) == 1)
        %Limiting:
        k = [y(2) * 0.5, 1000, 10000];
        %Killing:
        %y(2)=y(2)*.99; %Reducing the treatment sensitive subpopulation
